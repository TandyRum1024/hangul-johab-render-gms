///hj_comp_set(hanWidth, hanHeight, asciiWidth, asciiHeight, specialMiddle, specialLast)
/*
    [호환용/hj_draw_comp()]
    폰트 설정을 바꿉니다.
...
*/
global.hjCompOffMiddle = global.hjCompOffFirst + (global.hjCompBeolFirst * 28); // 중성 오프셋
global.hjCompOffLast = global.hjCompOffMiddle + (global.hjCompBeolMiddle * 28); // 종성 오프셋
global.hjCompOffJamo = global.hjCompOffLast + (global.hjCompBeolLast * 28); // 자모 오프셋
global.hjCompOffAscii = global.hjCompOffJamo + (global.hjCompBeolJamo * 28); // ASCII 오프셋
...
for (var i=8; i<=19; i++)
    global.hjComp_LUT_BEOL_MID[i] = 28;
...
global.hjComp_LUT_BEOL_LAST[0] = 28;
