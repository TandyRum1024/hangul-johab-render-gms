...
for (var i=1; i<=_strlen; i++)
{
    ...
    else // 한글
    {
        ...
        if (_curord >= $AC00 && _curord <= $D7AF) // 조합
        {
            ...
            TODO: FIX MIDDLE PIECE ERROR
            iui_label(_dx, _dy - 16, string(_mid), c_red);
            
            if (!global.hj_LUT_BEOL_SPECIAL[_mid])
                iui_rect(_dx, _dy, global.hjCharWidKor, global.hjCharHeiKor, c_red);
