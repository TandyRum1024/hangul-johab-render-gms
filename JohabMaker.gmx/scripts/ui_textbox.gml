...
if (mustTrim)
{
    iui_label_alpha(textOffX, textOffY, tmpChar, iuiColTextBoxText, 0.75);
    textOffX += string_width(tmpChar);
    tmpChar   = string_char_at(currentText, (showPos + displayChars) + 1);
    
    iui_label_alpha(textOffX, textOffY, tmpChar, iuiColTextBoxText, 0.5);
    textOffX += string_width(tmpChar);
    tmpChar   = string_char_at(currentText, (showPos + displayChars) + 2);
    
    iui_label_alpha(textOffX, textOffY, tmpChar, iuiColTextBoxText, 0.25);
}
...
#define ui_textbox_hangul
///ui_textbox_hangul(x, y, w, h, text, ID)
/**
    iui_textbox() but with hangul
    uses kia_*
**/
...
var showPos   = iui_varmap[? IDSTR + "_showpos"];
var cursorPos = iui_varmap[? IDSTR + "_cursorpos"];
var inHangul = iui_varmap[? "input_hangul"];
...
if (isFocus)
{
    ...
    {
        inHangul = !inHangul;
        iui_varmap[? "input_hangul"] = inHangul;
    }
    ...
    else if (inputCode >= 32 && inputCode <= 255) // write
    {
        ...
        {
            // Input normal
            // Code directly from KIA's example
            var tmpChar = "";
            var word = iui_keyChar;
            var idx = ord(string_lower(word)) - 97;
            
            if (inHangul && idx >= 0 && idx <= 25)
            {
                var _in = kia_input(idx);
                tmpChar += _in;
            ...
            currentText = string_insert(tmpChar, currentText, cursorPos + 1);
        }
    }
}
...
if (isFocus)
{
    if (inHangul && kia_type != "") // cursor & incomplete hangul
    {
        var _typeWid = string_width(kia_type);
        iui_rect_alpha(bx + (cursorX - 2) + 10, by + 10, _typeWid, bh - 20, textColour, 1); // cursor
        iui_label(bx + (cursorX - 2) + 10, textOffY, kia_type, ~textColour); // hangul glyph
    }
    else
        iui_rect_alpha(bx + (cursorX - 2) + 10, by + 10, 4, bh - 20, textColour, sin(iuiAnimTime * 0.1));
}
...
#define ui_textbox_numberonly
///ui_textbox_numberonly(x, y, w, h, text, ID, min_bound)
/**
    iui_textbox_numberonly()
**/
