...
global.hjGlpyhLineheight = 1; // 줄 간격 (줄 높이 제외)
global.hjGlyphJamoBeol = 0; // 초성 / 자모만을 그릴때 어느 벌을 사용할 지 정하는 변수
global.hjOffBody = (8 * 28) + 1; // 중성 오프셋
...
*/
    // TODO : 자모를 위한 테이블도 제작
// 중성 : 받침 없는 벌
global.hjLUTBody = hj_gen_table(-1, "ㄱㅋ", 0);
global.hjLUTBody = hj_gen_table(global.hjLUTBody, "ㄲㄴㄷㄸㄹㅁㅂㅃㅅㅆㅇㅈㅉㅊㅌㅍㅎ", 1 * 28); // 더 간편한 인덱싱을 위해 미리 28(한 줄의 스프라이트 갯수)를 곱합니다.

// 중성 : 받침 있는 벌
global.hjLUTBodyWithTail = hj_gen_table(-1, "ㄱㅋ", 2 * 28);
global.hjLUTBodyWithTail = hj_gen_table(global.hjLUTBodyWithTail, "ㄲㄴㄷㄸㄹㅁㅂㅃㅅㅆㅇㅈㅉㅊㅌㅍㅎ", 3 * 28);
...
// 종성
global.hjLUTTail = hj_gen_table(-1, "ㅏㅑㅘ", 0);
global.hjLUTTail = hj_gen_table(global.hjLUTTail, "ㅓㅕㅚㅝㅟㅢㅣ", 1 * 28);
global.hjLUTTail = hj_gen_table(global.hjLUTTail, "ㅐㅒㅔㅖㅙㅞ", 2 * 28);
global.hjLUTTail = hj_gen_table(global.hjLUTTail, "ㅗㅛㅜㅠㅡ", 3 * 28);
